...
class Hand {
	...
	int weaker=0;    // this is the pair in a full house of the weaker of two pairs in the two pair hand
	int kicker =0;
	int kicker1 = 0;
	...
	boolean checkPair() {
		...
		for(int i=1; i<5; i++) {
			if(sorted[i] == sorted[i-1]) {
				...
				for(int j=0; j<5 ; j++) {
					if(j!=i && j!=i-1) {
						kickers[j] = sorted[j]; // to find the kicker, iterates through the sorted list to save all of the values that aren't in the pair in the kickers array
